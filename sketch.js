...
function setup() {
  ...
  mario = createSprite(width-570 , height-150   ,20,50);
 ...
 invisiground = createSprite(width-50,height-70,  1200,10);
...
text("life: "+ life , 500,60);
  if (gameState===PLAY){
   // score = score + Math.round(getFrameRate()/60);
    ...
    if(score >= 0){
    invisiground.velocityX = -6;
    }else{
     invisiground.velocityX = -(6 + 3*score/100);
    }
...
function spawnCoin() {
  ...
  if (frameCount % 60 === 0) {
    var coin = createSprite(width-70,height-150,40,10)
